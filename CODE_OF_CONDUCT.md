# Code of Conduct

## Our Standards

Glyndor projects are technical projects. Interactions here — issues, PRs,
comments — should be focused, respectful, and constructive.

**Expected behavior:**

- Be direct and technically precise
- Critique code and ideas, not people
- Accept feedback gracefully — the goal is a better project
- Help others when you can

**Unacceptable behavior:**

- Harassment, personal attacks, or discriminatory language
- Deliberate intimidation or trolling
- Publishing others' private information without consent
- Any conduct that would be considered unprofessional in a technical setting

## Enforcement

Report violations privately to the maintainers through GitHub
([report abuse](https://github.com/contact/report-abuse) or the repository's
Security tab for sensitive reports). Reports are reviewed promptly and handled
with discretion. Maintainers may remove comments, close issues, or ban
contributors who violate these standards.

## Scope

This Code of Conduct applies to all project spaces — GitHub issues, PRs,
and any other official project channels.

---

*Based on the [Contributor Covenant](https://www.contributor-covenant.org/),
adapted for Glyndor.*
